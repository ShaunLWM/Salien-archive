...
{
	var rgParams = { url: 'http://localhost/salien/' + path };
	if ( method )
		rgParams.type = method;

	// if this js file was hosted off the store, add CORS request headers
	if ( window.location.href.indexOf( 'http://localhost/salien/' ) != 0 )
	{
...
function UpdateNotificationCounts()
{
	...
	if ( $NotificationItems.length || $NotificationTotalCounts.length )
	{
		$J.ajax( GetDefaultCommunityAJAXParams( 'GetNotificationCounts.php', 'GET' ) ).done( function( data ) {
